...
def time_slice_begin(tt, new_time_base):
    start_time = tt.start_time() + (new_time_base - tt.start_time() % new_time_base) * (
            tt.start_time() % new_time_base != 0)
    stop_time = tt.end_time() - tt.end_time() % new_time_base
    i = (tt.time >= start_time) * (tt.time < stop_time)
    return TimeTensor(tt.data[i, :], tt.time[i])


def time_rounding(tt: TimeTensor, step_size) -> TimeTensor:
    time, index = np.unique(step_size * np.round(tt.time / step_size), return_index=True)
    data = tt.data[index]
