...
def rolling_window(a, window):
    ...
    shape = (int(a.shape[0] / window), a.shape[1], window)
    strides = (window * a.strides[0],) + (a.strides[1],) + ( a.strides[0],)
    return np.lib.stride_tricks.as_strided(a, shape=shape, strides=strides)


def sliding_window_consecutive(time_tensor: TimeTensor, window_function, window_size: int = None) -> TimeTensor:
    '''
    This function run a sliding window function over the time tensor and
    return the result in a new time tensor, The window function input is a ndarray of size NxM
    where M is the dim of the current time tensor and N is the number of samples in the current calculation.
    ...
    :return:
    '''
    new_data = window_function(rolling_window(time_tensor.data, window_size))
    new_time = rolling_window(time_tensor.time.reshape(-1, 1), window_size)[:, 0, 0]
    return TimeTensor(new_data,
                      new_time)  # return a new time tensor after the sliding window
